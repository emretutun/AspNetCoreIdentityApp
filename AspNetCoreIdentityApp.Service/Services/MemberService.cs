...
namespace AspNetCoreIdentityApp.Service.Services
{
    public class MemberService : IMemberService
    {
        ...
        Task<(bool, IEnumerable<IdentityError>?)> IMemberService.ChangePasswordAsync(string userName, string oldPassword, string newPassword)
        {
            return ChangePasswordAsync(userName, oldPassword, newPassword);
        }

        Task<UserEditViewModel> IMemberService.GetUserEditViewModelAsync(string userName)
        {
            return GetUserEditViewModelAsync(userName);
        }
...

        {
            ...
            if (request.Picture != null && request.Picture.Length > 0)
            {
                ...
                var newPicturePah = Path.Combine(wwwrootFolder!.First(x => x.Name == "userpictures").PhysicalPath!, randomFileName);

                using var stream = new FileStream(newPicturePah, FileMode.Create);
